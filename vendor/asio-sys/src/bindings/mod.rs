// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  COPIE LOCALE PATCHÉE DE `asio-sys` 0.2.6 (Apache-2.0) — Jamodio 18/09/26 ║
// ╚══════════════════════════════════════════════════════════════════════════╝
//
// POURQUOI. Un pilote ASIO dispose de plusieurs façons de dire qu'il va mal.
// Le crate d'origine n'en transmet qu'UNE (`kAsioResetRequest`) : les autres
// sont interceptées et jetées (`TODO: Handle this`), et le changement de sample
// rate part dans un `eprintln!` vers une sortie d'erreur que personne ne lit en
// production. Résultat constaté : après deux épisodes de son dégradé sur un
// Focusrite (18/09/2026), impossible de savoir si le pilote avait signalé quoi
// que ce soit — par construction, pas par manque de chance.
//
...
/// PATCH JAMODIO — `(nombre de changements depuis le boot, dernier rate annoncé)`.
/// `(0, 0)` = le pilote n'a jamais rien annoncé.
pub fn sample_rate_change_report() -> (u64, u32) {
    use std::sync::atomic::Ordering;
    (
        SAMPLE_RATE_CHANGES.load(Ordering::Relaxed),
        LAST_REPORTED_RATE_HZ.load(Ordering::Relaxed),
    )
}
...
extern "C" fn sample_rate_did_change(s_rate: c_double) {
    ...
    use std::sync::atomic::Ordering;
    SAMPLE_RATE_CHANGES.fetch_add(1, Ordering::Relaxed);
