        ...
        loop {
            ...
            if !stream_data.is_empty() || has_self_signal || iso_changed {
                ...
                let msg = AgentMessage::StreamLevels {
                    ...
                    voice_on_air: Some(voice_on_air),
                    isolation_active: Some(isolation_active),
                };
            ...
            let peers: Vec<PeerPerf> = mixer_stats
                ...
                .map(|s| {
                    ...
                    PeerPerf {
                        ...
                        wait_deadline_unarmed: net.wait_deadline_unarmed,
        ...
        BrowserMessage::GetStats => {
            ...
            vec![
                ...
                AgentMessage::Stats {
                    ...
                    input_hw_ms: pl.input_hw.map(|d| d.hw_ms),
                    input_hw_source: pl.input_hw.map(|d| d.source()),
                    input_hw_kind: pl.input_hw.and_then(|d| d.bench).map(|b| b.kind),
                    input_hw_declared_ms: pl.input_hw.and_then(|d| d.bench).map(|b| b.declared_hw_ms),
